...
## 1. Active Core Workflow (Unified & Production-Ready)

The platform has successfully pivoted from academic tracking to a pure, enterprise-grade B.Tech Career Intelligence Platform. The closed-loop validation flywheel is fully operational across both the frontend and backend:
...
graph TD
    ...
    G --> H[Gamified Roadmap Learning & Practice Tasks]
    H --> I[Quick Screen Micro-Interviews / GitHub Ingestion]
    I --> F
```

### Path Components & Closed-Loop Integration
1. **Resume Ingestion & Parsing**: Extract plaintext resume structure using `/api/interview/sessions/parse-resume`.
2. **WebSocket Prep Room**: Students start dynamic, full-duplex WebSocket interview sessions with voice-to-text input (Web Speech API) and real-time LLM-driven evaluations.
3. **Interactive & Gamified Roadmaps**:
   - **Learn & Practice Nodes**: Clicking complete launches a modal overlay (*"Ready to lock in your progress? Take a 3-minute quick screen to officially unlock this skill badge."*), routing students to a 3-question WebSocket micro-session.
   - **Apply Nodes**: Integrates a GitHub repository input field that validates `github.com` structures. Submission triggers asynchronous complexity analysis.
4. **Real-Time Polling & Toasts**: While verification is `"⚙️ Analysis Pending"`, the frontend short-polls the backend and flashes a premium green toast upon successful evaluation: `"🎉 Project Verified! Your project weight has been upgraded!"`
