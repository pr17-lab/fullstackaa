...
def main():
    ...
    for user in users_with_student_id:
        try:
            ...
            if not password or len(password) == 0:
                errors.append(f"Empty password for user {user.id}, student_id: {user.student_id}")
                continue
            ...
            errors.append(f"Error for student_id {user.student_id}: {str(e)}")
            if len(errors) <= 5:
                print(f"    [!] {errors[-1]}")
    ...
    if sample and sample.password_hash and sample.password_hash.startswith("$2b$"):
        print(f"\n[+] Verification: Passwords use bcrypt format")
